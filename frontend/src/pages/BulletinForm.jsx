...
// // ✅ Calcul du TOTAL ANNUEL FIXE (addition de tous les SCL) = 265
// const TOTAL_ANNUEL = (() => {
//     let total = 0;
...
// console.log("📊 TOTAL ANNUEL FIXE =", TOTAL_ANNUEL); // 265
...
//     // ✅ Utilisation de SEQUENCE_OPTIONS au lieu de MONTH_OPTIONS
...
// // ✅ Calcul des totaux avec TOTAL ANNUEL FIXE = 265
// const calculateTotals = () => {
//     let sumNotes1 = 0, sumNotes2 = 0, sumNotes3 = 0;

//     // On garde les SCL par séquence pour information (optionnel)
...
//             sumSCL2, 
//             sumSCL3,
//             totalFixe: TOTAL_ANNUEL // 265
//         }
//     };
// };

// // ✅ Calcul des moyennes sur 20 basé sur le total FIXE (265)
// const calculateAverages = (totals) => {
//     const { sumNotes1, sumNotes2, sumNotes3, totalFixe } = totals.raw;

//     // Moyenne sur 20 = (notes obtenues / 265) × 20
//     const a1 = totalFixe > 0 ? Math.round((sumNotes1 / totalFixe) * 20 * 100) / 100 : 0;
//     const a2 = totalFixe > 0 ? Math.round((sumNotes2 / totalFixe) * 20 * 100) / 100 : 0;
//     const a3 = totalFixe > 0 ? Math.round((sumNotes3 / totalFixe) * 20 * 100) / 100 : 0;

//     return {
//         a1: totalFixe > 0 ? `${a1}/20` : "0/20",
//         a2: totalFixe > 0 ? `${a2}/20` : "0/20",
//         a3: totalFixe > 0 ? `${a3}/20` : "0/20",
//         raw: { a1, a2, a3 }
//     };
// };
...
//                                 <th className="border p-2 w-80">Skills</th>
...
// ✅ Calcul du TOTAL ANNUEL FIXE (addition de tous les SCL)
const TOTAL_ANNUEL = (() => {
    let total = 0;
    SKILLS_TEMPLATE.forEach(skill => {
        Object.values(skill.sclValues).forEach(value => {
...
export default function BulletinForm() {
    ...
    // ✅ Utilisation de SEQUENCE_OPTIONS
    const [periodHeaders, setPeriodHeaders] = useState({
        h1: SEQUENCE_OPTIONS[0],
        h2: SEQUENCE_OPTIONS[1],
        h3: SEQUENCE_OPTIONS[2]
    ...
    // ✅ Calcul des totaux avec TOTAL ANNUEL FIXE
    const calculateTotals = () => {
        let sumNotes1 = 0, sumNotes2 = 0, sumNotes3 = 0;
        let sumSCL1 = 0, sumSCL2 = 0, sumSCL3 = 0;
                ...
                <div className="flex flex-col md:flex-row gap-6  items-start">
                    ...
                    <div className="flex flex-row items-center gap-4 mb-1">
                        ...
                        <div className="flex-shrink-0">
                            <div className="w-35 h-20 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50 relative">
                                {studentPhoto ? (
                                    <>
                                        <img
                                            src={studentPhoto}
                    ...
                    <table className="w-full text-xl border-collapse">
                        ...
                        <thead>
                            <tr className="bg-gray-200">
                                <th className="border p-3 w-90 text-sm">Skills</th>
                                <th className="border p-3 w-100 text-sm">Description</th>
                                <th className="border p-3 text-center w-30 text-sm">Evaluation</th>
                                <th className="border p-3 text-center w-20 text-sm">SCl</th>
                                <th className="border p-3 text-center w-24 text-sm">
                                ...
                                <th className="border p-2 text-center w-100 text-sm">Appreciation</th>
                            </tr>
                        </thead>
                ...
                <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="border rounded p-4 text-lg">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-lg mb-3">
                            <div>
                                <select className="w-full border px-2 py-1 rounded text-xs" value={periodHeaders.h1} onChange={e => changeHeader("h1", e.target.value)}>
                                    {SEQUENCE_OPTIONS.map(m => <option key={m}>{m}</option>)}
                                </select>
                            </div>
                            <div>
                                <select className="w-full border px-2 py-1 rounded text-xs" value={periodHeaders.h2} onChange={e => changeHeader("h2", e.target.value)}>
                                    {SEQUENCE_OPTIONS.map(m => <option key={m}>{m}</option>)}
                                </select>
                            </div>
                            <div>
                                <select className="w-full border px-2 py-1 rounded text-xs" value={periodHeaders.h3} onChange={e => changeHeader("h3", e.target.value)}>
                                    {SEQUENCE_OPTIONS.map(m => <option key={m}>{m}</option>)}
                                </select>
                            </div>
                        </div>
