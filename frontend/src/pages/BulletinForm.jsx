...
//         Oral: 10, 
//         Practical: 15, 
//         Written1: 15,  // 👈 Première valeur (s'écrit en BD)
//         Written2: 20   // 👈 Deuxième valeur (s'écrit en BD)
//     },
//     evaluations: ["Attitude", "Oral", "Practical", "Written1", "Written2"]
// },
//     {
//         key: "2B",
...
//     {
//         key: "6B",
//         title: "6-B Science and Technology",
...
//             console.log("⚠️ skillData manquant pour", skill.key);
//             return;
//         }
...
//             if (!evalData) {
//                 console.log("⚠️ evalData manquant pour", ev, "dans", skill.key);
//                 return;
//             }
...
//                 sumSCL1 += skill.sclValues[ev] || 0; // ✅ Sécurité supplémentaire
//             }

//             // Période 2
//             if (evalData.m2 !== "" && !isNaN(evalData.m2)) {
//                 sumNotes2 += Number(evalData.m2);
//                 sumSCL2 += skill.sclValues[ev] || 0; // ✅ Sécurité supplémentaire
//             }

//             // Période 3
//             if (evalData.m3 !== "" && !isNaN(evalData.m3)) {
//                 sumNotes3 += Number(evalData.m3);
//                 sumSCL3 += skill.sclValues[ev] || 0; // ✅ Sécurité supplémentaire
//             }
//         });
//     });
...
//     if (ipcRenderer) {
//         ipcRenderer.send('print-bulletin', { type: 'anglophone', data: printData });
//     } else {
//         window.open(`${window.location.origin}/#/print-bulletin`, '_blank');
//     }
// };
...
// <React.Fragment key={skill.key}>
//     {skill.evaluations.map((ev, i) => {
//         // ✅ Vérification que state et evals existent
...
const calculateTotals = () => {
    ...
    return {
        // ✅ Total par séquence (dénominateur = TOTAL ANNUEL)
...
// Calcul des moyennes sur 20 (basé sur TOTAL ANNUEL)
const calculateAverages = (totals) => {
            ...
            <div className="bg-white w-full max-w-6xl rounded-xl shadow-lg p-6">
                        ...
                        <thead>
                            <tr className="bg-gray-100">
                                ...
                                <th className="border p-2 text-center w-24">
                                    ...
                                    {/* <div className="text-[9px] text-gray-500">Sequence 1</div> */}
                                </th>
                                <th className="border p-2 text-center w-24">
                                    <select className="text-xs" value={periodHeaders.h2} onChange={(e) => changeHeader("h2", e.target.value)}>
                                        {SEQUENCE_OPTIONS.map(m => <option key={m}>{m}</option>)}
                                    </select>
                                    {/* <div className="text-[9px] text-gray-500">Sequence 2</div> */}
                                </th>
                                <th className="border p-2 text-center w-24">
                                    <select className="text-xs" value={periodHeaders.h3} onChange={(e) => changeHeader("h3", e.target.value)}>
                                        {SEQUENCE_OPTIONS.map(m => <option key={m}>{m}</option>)}
                                    </select>
                                    {/* <div className="text-[9px] text-gray-500">Sequence 3</div> */}
                                </th>
                                <th className="border p-2 text-center w-28">Appreciation</th>
                            </tr>
                        </thead>
                ...
                <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="border rounded p-4 text-sm">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-xs items-center mb-2">
                            <div className="text-center">
                                <div className="text-gray-600">Total sequence-1</div>
                                <div className="font-semibold mt-1">{totals.t1}</div>
                                <div className="text-xs text-gray-400">Total sequence: {TOTAL_ANNUEL} pts</div>
                            </div>
                            <div className="text-center">
                                <div className="text-gray-600">Total sequence-2</div>
                                <div className="font-semibold mt-1">{totals.t2}</div>
                                <div className="text-xs text-gray-400">Total sequence: {TOTAL_ANNUEL} pts</div>
                            </div>
                            <div className="text-center">
                                <div className="text-gray-600">Total sequence-3</div>
                                <div className="font-semibold mt-1">{totals.t3}</div>
