...
// ============ GESTION DE L'IMPRESSION ELECTRON - VERSION SIMPLIFIÉE ============
// Impression directe - sans route, sans nouvelle fenêtre
ipcMain.on('print-bulletin', (event, data) => {
    console.log('🖨️ Impression directe demandée');
    
    // Récupérer la fenêtre principale
    const win = BrowserWindow.getFocusedWindow();
    
    if (!win) {
        console.error('❌ Aucune fenêtre trouvée pour l\'impression');
        return;
    }
    
    // Imprimer directement le contenu de la fenêtre actuelle
    win.webContents.print({
        silent: false,        // false = afficher la boîte de dialogue, true = impression silencieuse
        printBackground: true  // Imprimer les couleurs et arrière-plans
    }, (success, errorType) => {
        if (success) {
...
ipcMain.on('print-annual', (event, data) => {
    ...
    if (!win) {
        console.error('❌ Aucune fenêtre trouvée');
        return;
    }
    
    // Imprimer directement la fenêtre active
    win.webContents.print({
        silent: false,
        printBackground: true
    }, (success) => {
        console.log(success ? '✅ Impression annuelle lancée' : '❌ Échec impression annuelle');
    });
});
