...
// // Pour les photos, on utilise le dossier AppData pour l'écriture
// const photosAppDataDir = path.join(userDataPath, 'ecole details');
// if (!fs.existsSync(photosAppDataDir)) {
//     fs.mkdirSync(photosAppDataDir, { recursive: true });
//     console.log('📁 Dossier photos créé dans AppData:', photosAppDataDir);
// }

// // On garde aussi le dossier public pour la lecture en développement
// const ecoleDetailsDir = path.join(FRONTEND_PUBLIC_PATH, 'ecole details');
// if (!fs.existsSync(ecoleDetailsDir)) {
//     try {
//         fs.mkdirSync(ecoleDetailsDir, { recursive: true });
//         console.log('📁 Dossier ecole details créé dans public');
//     } catch (error) {
//         console.log('⚠️ Pas de permission pour créer dans public, utilisation AppData uniquement');
//     }
// }
...
//     try {
//         // Chercher d'abord dans AppData, puis dans public
//         let sourceDir = photosAppDataDir;
//         if (!fs.existsSync(sourceDir) || fs.readdirSync(sourceDir).length === 0) {
//             if (fs.existsSync(ecoleDetailsDir)) {
//                 sourceDir = ecoleDetailsDir;
//                 console.log('📁 Utilisation du dossier public pour l\'import');
//             } else {
//                 console.log('📁 Aucun dossier de photos trouvé');
//                 return;
//             }
//         }
...
//         const folders = fs.readdirSync(sourceDir).filter(item => {
...
//             const classFolder = path.join(sourceDir, folderName);
//             const files = fs.readdirSync(classFolder);
//             const imageFiles = files.filter(file => /\.(jpg|jpeg|png|gif|webp)$/i.test(file));
...
//                             // Copier la photo vers AppData si elle vient du dossier public
//                             let photoPath = `ecole-details/${folderName}/${file}`;
//                             if (sourceDir === ecoleDetailsDir) {
//                                 // Copier vers AppData pour la pérennité
//                                 const destDir = path.join(photosAppDataDir, folderName);
//                                 if (!fs.existsSync(destDir)) {
//                                     fs.mkdirSync(destDir, { recursive: true });
//                                 }
//                                 const destPath = path.join(destDir, file);
//                                 fs.copyFileSync(path.join(sourceDir, folderName, file), destPath);
//                                 photoPath = `ecole-details/${folderName}/${file}`;
//                             }
...
// // ============ ROUTES POUR LES PHOTOS ============
// // Route pour les photos dans AppData (prioritaire)
// appExpress.get('/ecole-details/*', (req, res) => {
//     // D'abord chercher dans AppData
//     const appDataPath = path.join(photosAppDataDir, req.params[0]);
//     if (fs.existsSync(appDataPath)) {
//         return res.sendFile(appDataPath);
//     }
    
//     // Sinon chercher dans public
//     const publicPath = path.join(FRONTEND_PUBLIC_PATH, 'ecole details', req.params[0]);
...
// appExpress.get('/temp/*', (req, res) => {
//     const filePath = path.join(tempDir, req.params[0]);
//     if (fs.existsSync(filePath)) {
//         res.sendFile(filePath);
//     } else {
//         res.status(404).json({ error: 'Photo temporaire non trouvée' });
...
// // ============ ROUTE CORRIGÉE POUR L'AJOUT DES ÉLÈVES ============
// appExpress.post('/api/student', (req, res) => {
//     try {
//         const { first_name, last_name, class_id, sex = '', photo_path = '' } = req.body;
//         if (!first_name || !last_name || !class_id) return res.status(400).json({ error: 'Données manquantes' });
...
//                 // Utiliser AppData pour sauvegarder (toujours accessible en écriture)
//                 const classDir = path.join(photosAppDataDir, class_name);
...
//                 // Copier le fichier (plus sûr que rename)
//                 fs.copyFileSync(tempPath, finalPath);
//                 // Supprimer le fichier temporaire
//                 fs.unlinkSync(tempPath);
                
//                 finalPhotoPath = `ecole-details/${class_name}/${newFilename}`;
//                 console.log(`📸 Photo sauvegardée dans: ${finalPath}`);
//             }
//         }
...
// ============ GESTION DE L'IMPRESSION ELECTRON ============
// Gestionnaire d'impression pour les bulletins
ipcMain.on('print-bulletin', (event, { type, data }) => {
    console.log(`🖨️ Impression ${type} demandée`);
    ...
    // Déterminer la route d'impression selon le type
    let printRoute = 'print-bulletin';
    if (type === 'francophone') printRoute = 'print-bulletin-francophone';
    else if (type === 'maternelle') printRoute = 'print-bulletin-maternelle';
    else if (type === 'annuel') printRoute = 'print-annuel';
    else if (type === 'annual') printRoute = 'print-annual';
    
    // Construire l'URL
    let printUrl;
    if (isDev) {
        printUrl = `http://localhost:5173/#/${printRoute}`;
    } else {
        // En production, charger le fichier avec le hash
        const indexPath = path.join(FRONTEND_DIST_PATH, 'index.html');
        printUrl = `file://${indexPath}#/${printRoute}`;
    }
    
    console.log('📂 Chargement de:', printUrl);
    
    // Quand la page est chargée
    printWindow.webContents.on('did-finish-load', () => {
        console.log('✅ Page d\'impression chargée');
        
        // Passer les données à la fenêtre d'impression via localStorage
        printWindow.webContents.executeJavaScript(`
            localStorage.setItem('printBulletinData', ${JSON.stringify(JSON.stringify(data))});
        `).then(() => {
            ...
            setTimeout(() => {
                ...
                printWindow.webContents.print({
                    ...
                    if (success) {
                        ...
                        console.log('❌ Erreur impression:', errorType);
                    }
    ...
    printWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription) => {
        console.error('❌ Erreur chargement page impression:', errorDescription);
        printWindow.close();
    });
...
// Gestionnaire pour le bulletin annuel anglais
ipcMain.on('print-annual', (event, data) => {
    console.log('🖨️ Impression bulletin annuel (anglais)');
    ...
    let printUrl;
    if (isDev) {
        printUrl = 'http://localhost:5173/#/print-annual';
    } else {
        const indexPath = path.join(FRONTEND_DIST_PATH, 'index.html');
        printUrl = `file://${indexPath}#/print-annual`;
    }
...
// Gestionnaire pour le bulletin annuel français
ipcMain.on('print-annuel', (event, data) => {
    console.log('🖨️ Impression bulletin annuel (français)');
    ...
    let printUrl;
    if (isDev) {
        printUrl = 'http://localhost:5173/#/print-annuel';
    } else {
        const indexPath = path.join(FRONTEND_DIST_PATH, 'index.html');
        printUrl = `file://${indexPath}#/print-annuel`;
    }
